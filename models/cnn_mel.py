...
class MEL_CNN(nn.Module):
    def __init__(self, *, cfg: ConfigCNN):
        ...
        pool2d = {PoolType.MAX: nn.MaxPool2d, PoolType.AVG: nn.AvgPool2d}.get(
            cfg.pool_type
        )
        ...
        for layer in cfg.conv_layers:
            if isinstance(layer, LayerConv):
                ...
                h, w = self._calculate_size(
                    h, w, layer.kernel_size, layer.stride, layer.padding
                )
        ...
        if cfg.global_avg_pool is not None:
            ...
            fc_input_size = current_channels * h * w
